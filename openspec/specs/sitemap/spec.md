...
Provide search engine crawlers with a discoverable list of indexable URLs so all hospital and specialty pages can be enumerated without depending on client-side routing. Backed by a backend `GET /sitemap.xml` endpoint plus a frontend `robots.txt` that points to it.
