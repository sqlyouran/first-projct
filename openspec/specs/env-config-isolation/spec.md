...
Isolate sensitive and environment-specific backend configuration (JWT secret, CORS origins, datasource credentials, mail settings) into environment variables so the same artifact can be deployed across dev / prod without changing source-controlled files. Local development continues to work with safe defaults; production fails fast if required variables are missing.
